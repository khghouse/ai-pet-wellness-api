...
## 작업 브랜치 및 Worktree 규칙

새 기능 또는 의미 있는 변경을 시작할 때는 최신 `main` 기준으로 작업 성격에 맞는 브랜치 이름을 결정하고 독립 Git worktree를 생성한다.

- 브랜치 이름은 `feature/`, `fix/`, `docs/`, `refactor/`, `chore/` 접두사를 사용한다.
- worktree는 저장소 상위 경로에 `{저장소명}-{작업명}` 형태로 생성한다.
- worktree 생성 전 현재 `main`의 변경 사항과 기존 worktree 목록을 확인한다.
- main이 깨끗하지 않거나 동일 작업 브랜치 또는 worktree가 이미 존재하면 임의로 처리하지 않고 사용자에게 알린다.
- 생성 후 작업 브랜치명과 worktree 절대 경로를 보고한다.
- 병렬 작업 중에는 각 에이전트가 자신에게 할당된 worktree만 수정한다.

사용자가 요구사항을 제공하고 작업 시작을 요청하면 AI 에이전트는 기본적으로 다음 범위까지 수행한다.

1. 현재 브랜치와 작업 트리 상태를 확인한다.
2. `main`을 최신화하고 작업 성격에 맞는 브랜치와 독립 worktree를 생성한다.
3. 관련 컨텍스트 문서를 갱신한다.

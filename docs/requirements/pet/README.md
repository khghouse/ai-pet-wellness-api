...
- **필드:**
  - `id`: 체중 이력 식별자
  - `pet`: 반려견
  - `weight`: 체중, kg 단위, `0.1` 이상 `999.9` 이하, 소수점 한 자리
  - `measuredAt`: 사용자가 입력한 실제 체중 측정 시각, `LocalDateTime`
  - `createdAt`: 체중 이력 등록 시각
- **정책:**
  - 체중을 기록할 때마다 새 이력을 생성한다.
  - 반려견 등록 시 입력한 체중은 첫 번째 이력으로 생성한다.
  - 체중 기록은 `OWNER`, `FAMILY` 역할의 `ACTIVE` 멤버십만 생성할 수 있다.
  - 측정 시각은 반려견 생년월일 당일 `00:00` 이후이고, 현재 한국 시간보다 미래가 아니어야 한다.

...
- 응답 DTO는 엔티티별 범용 DTO 하나가 아닌 API 목적별 DTO로 설계한다.
- 목록에는 `{도메인}SummaryResponse`, 상세에는 `{도메인}DetailResponse`처럼 필요한 데이터만 제공한다.
- 여러 응답에서 반복되는 작은 데이터는 `BreedResponse`, `PetSummaryResponse`처럼 재사용 가능한 응답 DTO로 분리할 수 있다.
- 연관 엔티티를 응답에 포함할 때는 해당 API에 필요한 요약 정보만 포함한다.
- 요청 경로나 인증 정보로 이미 알 수 있는 부모 리소스 또는 요청 회원 정보는 응답에서 중복하지 않는다.
- Entity는 API 응답에 직접 노출하지 않는다.

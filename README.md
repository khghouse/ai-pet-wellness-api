...
AI 에이전트 기반 개발 흐름과 하네스 엔지니어링을 학습하고, 그 위에서 기능을 점진적으로 구현하는 데 초점을 둡니다.
...
- ArchUnit: 레이어와 도메인 간 의존 규칙 검증
- Testcontainers: Redis가 필요한 인증 테스트를 컨테이너 환경에서 검증
- API 인수 시나리오 테스트: 실제 HTTP 서버에서 핵심 사용자 흐름 검증
- GitHub Actions: PR 및 `main` 브랜치 push 시 CI 빌드 검증

## 테스트 전략

- Controller 테스트: 요청과 응답 계약, 입력값 검증
- Service 통합 테스트: 도메인 정책과 DB 상태 변경
- 인증 통합 테스트: JWT, Redis, 보안 필터 흐름
- API 인수 시나리오 테스트: 실제 HTTP 기반 핵심 사용자 여정
- ArchUnit 테스트: 패키지와 레이어 의존 규칙
...
- `docs/adr/`: 아키텍처 결정 기록
- `docs/test-scenarios/`: 핵심 사용자 흐름과 API 인수 시나리오

AI 에이전트 작업 규칙은 [`AGENTS.md`](AGENTS.md)에서 관리합니다.
...
인증 통합 테스트와 API 인수 시나리오 테스트는 Testcontainers Redis를 사용하므로 로컬 실행 시 Docker Desktop이 필요합니다.
...
2. 최신 `main` 기준으로 작업 브랜치와 독립 Git worktree를 준비한다.
